...
const ModelCreateComic = (props) => {
  ...
  useEffect(() => {
    if (listGenres.length > 0) {
      setTheLoai(listGenres[0].mA_THE_LOAI); // Thiết lập giá trị mặc định từ thể loại đầu tiên
    }
  }, [listGenres]);

  useEffect(() => {
    if (listAuthor.length > 0) {
      setTenTacGia(listAuthor[0].mA_TAC_GIA); // Thiết lập giá trị mặc định từ tác giả đầu tiên
    }
  }, [listAuthor]);
  ...
  const handleClose = () => {
    ...
    setMota("");
    setGhichu("");
    setTheLoai(listGenres[0].mA_THE_LOAI);
    setTenTacGia(listAuthor[0].mA_TAC_GIA);
  };
  ...
  const handleSubmitCreateComic = async () => {
    ...
    console.log("Thể loại:", theloai, "Tác giả:", tentacgia);
    let data = await postNewComis(
      tentruyen,
      anhbia,
      noidung,
      tinhtrang,
        ...
        <Modal.Body>
          <form className="row g-3">
            ...
            <div className="col-md-6">
              ...
              >
                {listGenres.map((item) => (
                  <option key={item.mA_THE_LOAI} value={item.mA_THE_LOAI}>
                    {item.teN_THE_LOAI}
                  </option>
                ))}
              </select>
            ...
            <div className="col-md-6">
              ...
              >
                {listAuthor.map((item) => (
                  <option key={item.mA_TAC_GIA} value={item.mA_TAC_GIA}>
                    {item.teN_TAC_GIA}
                  </option>
                ))}
              </select>
